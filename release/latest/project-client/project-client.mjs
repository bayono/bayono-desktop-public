...
//#endregion
//#region src/scripts/project-client/project-config/bayono-project-file.ts
const BayonoProjectFileUtils = {
	makeNew: (data) => {
		return {
			name: data.name ?? "My Bayono Project",
			version: data.version ?? "0.1.0",
			description: data.description ?? "A new Bayono project"
		};
	},
	serdes: {
		serialize: (data) => {
		...
		deserialize: (content) => {
			try {
				...
				throw new Error("Failed to parse project file content: " + error);
			}
		}
	}
};
//#endregion
//#region src/scripts/project-client/project-client-init.ts
async function projectClientInit(dir = process.cwd()) {
	console.log("Initializing a new project...");
	const oldDir = process.cwd();
	const folderName = "bayono";
	const projectFileName = AppConstants.projectFilename;
	process.chdir(dir);
	...
	[
		...
		fs.mkdirSync(`${folderName}/${subdir}`, { recursive: true });
	});
	try {
		const projectFileContent = BayonoProjectFileUtils.makeNew({});
		const serializedContent = BayonoProjectFileUtils.serdes.serialize(projectFileContent);
		fs.writeFileSync(`${folderName}/${projectFileName}`, serializedContent);
	} catch (error) {
		console.error("Failed to create project file: " + error);
		return;
	}
	process.chdir(oldDir);
	console.log(`Project initialized successfully in the '${folderName}' directory.`);
}
//#endregion
...
const textFormat$1 = { path: (p) => {
	const homeDir = os$1.homedir();
	return p.replace(homeDir, "~");
} };
const formatters$1 = {
	path: (p) => chalk.underline.blue(textFormat$1.path(p)),
...
const CommsMessageUtils = { error: (msg, ...lines) => {
	formatters$1.error(Tags.error) + "" + msg + lines.flatMap((line) => indent(2) + line).join("\n");
...
//#endregion
//#region src/scripts/lib/sliver-tui-lib/sliver-tui/index.mjs
const cursorCodes = {
	hide: "\x1B[?25l",
	show: "\x1B[?25h",
...
const AppCommandSetupHelper = { cleanupCommandName(commands) {
	const cleaned = {};
	for (let [cmd, entry] of Object.entries(commands)) {
		cmd = cmd.length === 0 ? "cmd" : cmd;
		const cleanedCmd = cmd.trim().toLowerCase().replace(/\s+/g, "-");
		cleaned[cleanedCmd] = entry;
	}
	return cleaned;
} };
const AppPrintHelpers = { cleanLines(lines) {
	return lines.flatMap((line) => line.split("\n")).join(cursorCodes.clearNewLine);
} };
const ctrlC = "";
const ctrlD = "";
const AnsiKeyCodes = {
...
const toPrint = [];
const DrawSettings = {
	topVerticalPadding: 1,
	promptVerticalPadding: 0
};
const DrawSymbols = { border: "=" };
var App = class {
	____isAppHandle = true;
	lastDrawSize = 0;
	...
	draw() {
		...
		toPrint.length = 0;
		{
			const topPadding = this.drawSettings.topVerticalPadding;
			for (let i = 0; i < topPadding; i++) toPrint.push("");
		}
		...
		{
			for (let i = 0; i < this.drawSettings.promptVerticalPadding; i++) toPrint.push("");
			toPrint.push(drawUtils.horizontalLine(DrawSymbols.border));
		...
		{
			const bufferLines = this.printBuffer.flush();
			const cleanedLines = AppPrintHelpers.cleanLines(bufferLines);
			if (cleanedLines.length > 0) println(cleanedLines);
		}
		{
			const diff = Math.max(0, this.lastDrawSize - toPrint.length);
			print(cursorCodes.clearNewLine.repeat(diff));
		}
		print(AppPrintHelpers.cleanLines(toPrint));
		if (toPrint.length <= 0) {
			cursorUtils.clearToEndOfLine();
	...
	constructor(args = {}) {
		...
		const topUiElements = args.topUi?.map((fn) => fn({ appState })) ?? [];
		const bottomUiElements = args.bottomUi?.map((fn) => fn({ appState })) ?? [];
		if (args.banContinuousElements) {
			const continuousElements = [...topUiElements.filter((ui) => ui.characteristic.continuous), ...bottomUiElements.filter((ui) => ui.characteristic.continuous)];
		...
		this.appState = appState;
		this.topUi = topUiElements;
		this.bottomUi = bottomUiElements;
		...
		this.actionCommands = AppCommandSetupHelper.cleanupCommandName(args.commands ?? {});
	...
	handleCommandInstruction(instruction, cachedUiElements) {
		const uiElements = cachedUiElements ?? [...this.topUi, ...this.bottomUi];
		if (instruction.kind === "print") this.printBuffer.add(instruction.message);
		else if (instruction.kind === "updateUiElement") {
			const { elementLabel, data } = instruction;
			uiElements.filter((ui) => ui.label === elementLabel).forEach((ui) => {
				ui.handleMessage(data);
			});
		} else if (instruction.kind === "clearScreen") this.clear();
		else console.error(instruction);
	}
	handleMessage(rawText) {
		const text = rawText.trim().toLowerCase();
		const cmdRaw = rawText.split(" ")[0] ?? "";
		...
		if (text === "/clear") {
			...
			const otherOptions = Object.entries(this.actionCommands).map(([cmd, entry]) => [`${cmd}`, entry.description]);
			...
			this.printBuffer.add("");
			return;
		} else if (cmd && cmd in this.actionCommands) {
			const action = this.actionCommands[cmd];
			const uiElements = [...this.topUi, ...this.bottomUi];
			if (action) {
				const restOfText = rawText.slice(cmdRaw.length);
				action.action(this, restOfText).forEach((instruction) => {
					this.handleCommandInstruction(instruction, uiElements);
				});
			}
			return;
		} else if (text === "/exit") {
			...
			if (this.args.onUnknownCommand) this.args.onUnknownCommand(text).forEach((instruction) => {
				this.handleCommandInstruction(instruction);
			});
...
const AppUtils = { fromAppHandle(handle) {
	return handle;
} };
const bannerAscii = {
	text: `   _
...
//#endregion
//#region src/scripts/project-client/project-client-lib.json
var version = "0.1.0";
var project_client_lib_default = {
	version,
	install: "",
	connected: ""
};
...
const msg = (t) => `
Project Linked!
  - Path: ${formatters$1.path(t)}
  - Version: ${version}
`;
var StatusElement = class extends UiElement {
	label = "project-status";
	characteristic = { continuous: false };
	draw(ctx) {
		const message = [["●", formatters$1.greenText("●")], ["Connected", formatters$1.greenText("Connected")]];
		const msg = [];
		let msgSize = 0;
		const spaceSize = 1;
		for (const [text, formatted] of message) {
			if (msgSize + text.length + spaceSize > ctx.cols) break;
			msg.push(formatted);
			msgSize += text.length + spaceSize;
		}
		return [msg.join(" ")];
	}
};
var ProjectClient = class {
	...
	constructor(projectDir) {
		...
		this.tui = new App({
			printOnStart: message,
			topUi: [(args) => new StatusElement(args.appState)],
			commands: { info: {
				name: "info",
				description: "Display project information",
				action: (appHandle, args) => {
					AppUtils.fromAppHandle(appHandle);
					return [{
						kind: "print",
						message: JSON.stringify(project_client_lib_default, null, 2)
					}];
				}
			} }
		});
...
//#endregion
//#region src/scripts/project-client/project-directory.ts
var ProjectClientDirectory = class {
	rootDir;
	constructor(rootDir) {
		this.rootDir = rootDir;
	}
};
const ProjectClientDirectoryUtils = {
	checkFolder: async (rootDir) => {
		const projectFile = `${rootDir}/${AppConstants.projectFilename}`;
		if (!fs.existsSync(projectFile)) return {
			kind: "no-project-file",
			message: `No project file found in ${rootDir}`
		};
		try {
			await fs.promises.access(projectFile, fs.constants.R_OK);
			...
			console.error(error);
			return {
				kind: "invalid-project-file",
				message: `Invalid project file in ${rootDir}`
			};
		}
	},
	create: async (rootDir) => {
		if ((await ProjectClientDirectoryUtils.checkFolder(rootDir)).kind !== "ok") throw new Error("Invalid project directory: " + rootDir);
		return new ProjectClientDirectory(rootDir);
	}
};
//#endregion
//#region src/scripts/project-client/project-client-start.ts
const invalidFolderMsg = (p) => `
${formatters$1.error("[Error]")} Invalid project directory
  - path: ${formatters$1.path(p)}


${formatters$1.info("[Tip]")}

Check:
  - ${formatters$1.bold(AppConstants.projectFilename)} exists in the folder


${formatters$1.info("[Example]")}

${formatters$1.bold("Valid folder structure:")}
- MyProject
  └── bayono ${formatters$1.info("<-- Run from here")} 
    └── ${formatters$1.bold(AppConstants.projectFilename)}  ${formatters$1.error("<-- REQUIRED")}

`;
const noProjectFileMsg = (p) => `
${formatters$1.error("[Error]")} Project file not found
${formatters$1.info("[Tip]")} Please ensure the project directory contains a ${formatters$1.bold(AppConstants.projectFilename)} file.
...
`;
const invalidProjectFileMsg = (p) => `
${formatters$1.error("[Error]")} Invalid project file
`;
const invalidProjectFileTipMsg = (p) => `
${formatters$1.info("[Tip]")} Please ensure the project file is a valid JSON file with the required fields.
`;
const generalErrorMsg = (message) => `
${formatters$1.error("[Error]")} ${message}
`;
async function start(args = {}) {
	const projectDirPath = args.directory ?? cwd();
	args.noLog;
	{
		const isValid = await ProjectClientDirectoryUtils.checkFolder(projectDirPath);
		if (isValid.kind === "no-folder") {
			console.error(invalidFolderMsg(textFormat$1.path(projectDirPath)));
			process.exit(1);
		} else if (isValid.kind === "no-project-file") {
			console.error(noProjectFileMsg(textFormat$1.path(projectDirPath)));
			console.error(projectDirPath);
			process.exit(1);
		} else if (isValid.kind === "invalid-project-file") {
			console.error(invalidProjectFileMsg(textFormat$1.path(projectDirPath)) + invalidProjectFileTipMsg(textFormat$1.path(projectDirPath)));
			process.exit(1);
		} else if (isValid.kind === "error") {
			console.error(generalErrorMsg(isValid.message));
			process.exit(1);
		}
	}
	const projectDir = await ProjectClientDirectoryUtils.create(projectDirPath).catch((error) => {
		console.error(error instanceof Error ? error.message : String(error));
		return null;
	});
	if (!projectDir) {
		console.error("Failed to create project directory. Exiting.");
		process.exit(1);
	}
	checkDaemon: {
		if (await DaemonDevClientUtils.isUp()) break checkDaemon;
		const appDataPath = await AppDataUtils.getGlobalAppData();
		if (isCommsMessage(appDataPath)) {
			console.error("Failed to start Daemon");
			console.error("  - Reason: Unable to determine global app data path");
			console.error("  - Is 'bayono desktop' installed?");
			process.exit(1);
		}
		if (appDataPath.rootKind !== "global") console.error("Local app data not supported yet");
		console.log(`${formatters$1.info("[Info]")} Daemon server is not running. Attempting to start it...`);
		const daemonPath = appDataPathUtils.getDaemonPath(appDataPath.rootDir);
		console.log(`Starting daemon from: ${formatters$1.path(daemonPath)}`);
		spawn("node", [daemonPath], {
			detached: true,
			stdio: "ignore"
		}).unref();
	}
	new ProjectClient(projectDir).start();
}
//#endregion
//#region src/scripts/project-client/project-client-cli.ts
const ProjectClientCli = new Command("project-client").description("Commands related to the project client");
ProjectClientCli.command("start").description("Start the project client").action(async () => {
	start();
});
ProjectClientCli.command("init").description("Initialize a new project").action(async (opts) => {
	projectClientInit();
});
ProjectClientCli.parseAsync(process.argv);
//#endregion
export { ProjectClientCli };
